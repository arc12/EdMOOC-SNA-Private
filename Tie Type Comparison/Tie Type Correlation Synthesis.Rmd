...
biplot(pca, xlabs = rep("+", length(data_WT3PS$astro$wt[,1])), cex = 0.7, main = "The First Two Principle Components", sub="Whole-Thread with threshold=3")
```
...
Spearman rank correlation is used with square root measures; a non-linear but monotonic relationship will appear as a correlation. Spearman rank correlation is more suited to skew distributions and more resistant to the effect of outliers than Pearson's but the centrality measures cannot be assumed to conform to the assumptions of the correlation method. It should not be assumed that coefficients are quantitatively meaningful. 
