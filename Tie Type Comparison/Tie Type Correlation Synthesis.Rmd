...
plot(sim[,"sqrt_betweenness"],sim[,"sqrt_degree"], xlab=expression(sqrt("betweenness")), ylab=expression(sqrt("degree")), main="Simulated Erdős–Rényi Network", cex=0.6)
mtext("(matching Astrobiology Post-Set node & edge count)", cex=0.9)
plot(sim[,"sqrt_betweenness"],sim[,"sqrt_eigenvector"], xlab=expression(sqrt("betweenness")), ylab=expression(sqrt("eigenvector")), main="Simulated Erdős–Rényi Network", cex=0.6)
mtext("(matching Astrobiology Post-Set node & edge count)", cex=0.9)
```
...
WTsim_corr<- cor(sim<-data_WT3PS$astro$sim_wt, use="complete.obs", method="spearman")
df<-rbind(
...
When there are many components, selection often chooses principal components to the left of the elbow point in the scree plot. alternatively, PCs with eigenvalues >1 may be selected; these account for more variance than the original variables. On the basis of this recipe, we could select one or two principal components for further analysis. This has the distinct disadvantage in that it gives us statistics that are not directly interpretable in terms of ideas such as "degree" or "betweenness". Hence it may be necessary to transform the components back to centrality measure space, or simply to use the centrality measures where their non-independence is not an issue.
